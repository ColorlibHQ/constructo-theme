...
function constructo_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'constructo' ),
        'next_string'     => esc_html__( '&raquo;', 'constructo' ),
        'before_output'   => '<div class="constructo-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'constructo_pagination_defaults', $defaults )
    );
