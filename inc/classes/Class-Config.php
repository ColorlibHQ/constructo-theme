...
// Final Class
final class Constructo {

	// Theme Version
	private $constructo_version = '1.0';
	...
	private function setup() {
		...
		$enqueu          = new constructo_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->constructo_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'constructo_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'constructo', CONSTRUCTO_DIR_PATH . '/languages' );
		...
		// Site logo sizes
		add_image_size( 'constructo_site_logo_200x37', 200, 37, true );
		add_image_size( 'constructo_site_footer_logo_260x48', 260, 48, true );

		// About image sizes
		add_image_size( 'constructo_about_shape_155x185', 155, 185, true );
		add_image_size( 'constructo_about_img_540x606', 540, 606, true );

		// Service thumbnail size
		add_image_size( 'constructo_service_thumb_362x270', 362, 270, true );

		// Latest post thumbnail size
		add_image_size( 'constructo_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'constructo_single_blog_750x375', 750, 375, true );
		add_image_size( 'constructo_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu'	=> esc_html__( 'Primary Menu', 'constructo' ),
			'services'		=> esc_html__( 'Services', 'constructo' ),
			'company' 		=> esc_html__( 'Company', 'constructo' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = CONSTRUCTO_DIR_CSS_URI;
		$jsPath  = CONSTRUCTO_DIR_JS_URI;
		...
		$scripts = array(
			'style'   => array(
				...
				array(
					...
					'dependency' => array(),
					'version'    => $this->constructo_version,
				),
				array(
					'handler' => 'constructo-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'constructo' ) ) {
	...
	private function customizer_init(){
		...
		// Instantiate constructo theme customizer
		$constructo_theme_customizer = new constructo_theme_customizer();
	}
	...
	public function epsilon_quickie(){
			...
			'links' => array(
				array(
					'link_to'   => 'constructo_options_panel',
	...
	public function set_elementor_flag(){
		update_option( 'constructo_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'constructo-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'constructo_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'constructo-elementor-default-style',  CONSTRUCTO_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->constructo_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'constructo_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'constructo-elementor-notice', CONSTRUCTO_DIR_JS_URI.'constructo-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'constructo-elementor-notice',
				'constructoElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'constructo-elementor-notice-nonce' ),
				)
			);
		}
