...
function constructo_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'constructo' ) . '</span></li>';
...
// Blog categories
function constructo_featured_post_cat(){
...
// social media
if ( ! function_exists( 'constructo_social' ) ) {
	function constructo_social( $args = array() ) {
		...
		$url = constructo_opt( 'constructo_social_url' );
...
// header cart count
function constructo_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( CONSTRUCTO_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'constructo' ); ?>">
...
function constructo_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'constructo_contact7_form_content', 10, 2 );